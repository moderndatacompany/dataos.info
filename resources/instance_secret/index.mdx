...
<AccordionGroup>
  <Accordion title="Azure Blob File System Secure (ABFSS)" defaultOpen={false}>
    ...
    * **Use cases:** A developer can create an Instance Secret only if an operator assigns them the "Manage All Instance-level Resources of DataOS in the user layer" use case using Bifrost Governance. Alternatively an operator can assign the `data-dev` tag to a developer instead of assigning a use case.
    ...
    <Warning>
      Note that for read write access of an  Instance Secret the user have to create two Instance Secrets on with `acl:r` and other with `acl:rw` with similar names such as `abfssdepot-r `and` abfssdepot-rw`&#x20;
    </Warning>
