...
DataOS allows to create a Depot of type 'ICEBASE' to read the schemas/tables stored in DataOS’s lakehouse called Icebase. The created Depot enables the user to read the stored data in datasets, which helps in scanning metadata from the ICEBASE-type depot with the Scanner Workflows.
