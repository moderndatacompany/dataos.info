...
description: "This section provides a guide and reference for using the Bigquery metadata Scanner Workflow. It includes configuration steps, and required access permissions for execution."
---
...
2) **Access Permissions in GCP**: 
  To successfully execute the Metadata Extraction Scanner Workflow in GCP, the user or service account needs specific permissions to access and retrieve the required data. Here's a summary of the minimum required permissions:
...
3) **Access Permissions in DataOS**:
...
4) **Pre-created Bigquery Depot**:
  Ensure that a Bigquery Depot is already created with valid read access and the necessary permissions to extract metadata. To check the Depot go to the Metis UI of the DataOS or use the following command:
