...
* This pattern ensures that only databases starting with
...
* Using anchored regex (
  ...
  ) prevents unintentional matches.
...
* Avoid specifying the exact names of databases, schemas, or tables without using regex. Any provided name will automatically be converted into a regex pattern with a wildcard appended, meaning that "SNOWFLAKE" will be interpreted as
...
* `^SNOWFLAKE.*`
  ...
  , etc., leading to unintended scans.
