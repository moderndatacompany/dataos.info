...
3. **Create a Depot manifest file**After creating an Instance Secret, next step is to create a manifest file for Depot.
...
5. **Apply the Depot manifest file**Apply the Depot manifest file by executing the below command.```bash
   dataos-ctl apply -f /home/mssql_depot.yaml
   ```

6. **Validate the Depot**:
