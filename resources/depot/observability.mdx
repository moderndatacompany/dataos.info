...
### How to observe the Depot pod logs using the Operations app?
...
* **Pod Details**
  ...
  * **Name:** `depotservice-api-795976b667-c7djp`

  * **Namespace:** `depot`

  * **Data Plane:** `hub`

  * **Phase:** `Running` (indicating the pod is active and functioning)

* **Containers**

  * **Container Name:** `depotservice-api`

  * **Container Image:** `docker.io/rubiklabs/depot-service:0.8.54`

  * **Container Image Pull Policy:** `IfNotPresent`(ensures the image is only pulled if not already available locally)

* **Conditions** (Pod’s readiness and initialization status)
...
* **Container Status**
  ...
  * **Events** (No events are currently displayed, but this section would normally show logs related to the pod's lifecycle, errors, or warnings.)

1. Pod YAML contains the manifest file of the Pod.


   ![](/Screenshotfrom2025-02-1216-50-02.png)

2. Pod Logs contains the logs related to the Pod.


   ![](/Screenshotfrom2025-02-1216-50-34.png)

3. The Pod Usage panel in DataOS Operations provides insights into the resource consumption of running pods within a namespace. It helps monitor CPU and memory utilization over time, ensuring optimal performance and resource allocation.
