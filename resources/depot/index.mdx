...
<AccordionGroup>
  ...
  <Accordion title="Depot as Lakehouse" defaultOpen={false}>
    Whenever a user ingest the data, it get stored in the DataOS Lakehouse Depot.
  </Accordion>

  <Accordion title="Applying data Policies" defaultOpen={false}>
    To govern the data, a developer can create data policies using Policy Stack that restrict the access to the data for specific users.
  </Accordion>

  <Accordion title="Scanning meta data" defaultOpen={false}>
    After creating the Depot , a user can run a Scanner Workflow for Depot to extract the metadata which later can be accessed on Metis UI.
  </Accordion>

  <Accordion title="Limit the data source's file format" defaultOpen={false}>
...
* Do not delete a Depot if it has a dependency on other Resources.
