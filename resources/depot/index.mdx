---
title: "Overview"
description: "Depot in DataOS is a Resource used to connect different data sources to DataOS by abstracting the complexities associated with the underlying source system (including protocols, credentials, and connection schemas). "
sidebarTitle: "Overview"
---

It enables users to establish connections and retrieve data from various data sources, such as data lake systems (e.g., AWS S3, Google GCS, Azure Blob Storage), database systems (e.g., Redshift, SnowflakeDB, Bigquery, Postgres), and event systems (e.g., Kafka, Pulsar) without moving the data.
...
<Frame caption="Hierarchical Structure of a Data Source within DataOS">
...
<Frame caption="Depot manifest structure">
  ![](/depot_structure.jpg)
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Eventhub" defaultOpen={false}>
    ...
    * **Eventhub Shared Access Key**: The actual shared access key used to authenticate with EventHub. This key is found alongside the "Shared Access Key Name" in the "Shared Access Policies" section of your EventHub namespace settings in the Azure portal. Ensure that this key is securely stored and handled.
    ...
    <Warning>
      Please note that the credentials are directly specified in the Depot manifest using the `connectionSecret`, whereas credentials are referred to via [Instance Secret](https://dataos.info/resources/instance_secret/) as `secrets` or `dataosSecrets`.
    </Warning>
  ...
  <Accordion title="PostgreSQL" defaultOpen={false}>
    ...
    * **Password**: The password associated with the PostgreSQL username for authentication. This is set during user account creation and must be securely obtained from the database administrator if forgotten.
    ...
    <Warning>
      Use the below templates, if the self-signed certificate is enabled.
    </Warning>
    ...
    <Warning>
      Use the below templates, if the self-signed certificate is not enabled.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Building semantic models" defaultOpen={false}>
    A semantic model can be build using Depot as a source. To learn more about this please [refer to this link](https://dashboard.mintlify.com/dataosinfo/dataosinfo/editor/v4#data-ingestion-and-transformation) on creating a semantic model using  Snowflake Depot as a source. You can build a semantic model on Redshift, Bigquery, Postgres, Snowflake, and Lakehouse type of Depots. To create a semantic model on other types of Depots one must ingest data to DataOS Lakehouse using Flare Stack.
  </Accordion>
  ...
  <Accordion title="Accelerating queries" defaultOpen={false}>
    ...
    **For example:**
    ...
    The below manifest file defines a DataOS service named `flashtest`, built on the `flash+python:2.0` Stack. The Service caches a dataset named `city`, stored in `dataos://lakehouse:retail/city`, and initializes a table `mycity` by selecting data from `retail.city`.
  ...
  <Accordion title="Building Data Products" defaultOpen={false}>
    A Depot plays a fundamental role in Data Product development, a Data Product can be created directly from the source Depot or from the lakehouse (Lakehouse) Depot.
...
* Give your Depot a meaningful name that represents its purpose.**Example:**&#x54;his makes it easier to manage and identify Depots.
  ...
  * ✅ `s3depot` instead of ❌ `s3-1`

  * ✅ `dbmysql` instead of ❌ `db1`
...
The catalog of data sources accessible by one or more components within DataOS is provided on the following page: [Supported Connectors in DataOS](/resources/depot/supported_connectors/).

## **FAQs**

<AccordionGroup>
  <Accordion title="1. How do I know which data sources are supported in DataOS?" defaultOpen={false}>
    The complete list of supported connectors is available in the [**Data Integration - Supported Connectors**](/resources/depot/supported_connectors/) section.
  </Accordion>
  ...
  <Accordion title="8. Is there any way to know which Resources are using a Depot?" defaultOpen={false}>
    Currently, there no direct way to know that.
  </Accordion>
</AccordionGroup>