...
<AccordionGroup>
  <Accordion title="ABFSS" defaultOpen={false}>
    ...
    The GCS manifest includes a **Resource meta** section with essential metadata attributes common to all resource types. Some attributes in this section are optional, while others are mandatory. For more details, refer to the **Attributes of Resource Meta Section**.

    ### Instance-secret Specific Section

    This section focuses on attributes specific to GCS Instance Secrets. It includes the following details:

    * **type**: Specifies the Instance Secret type (key-value-properties).

    * **acl**: Access control level (read-only or read-write). You can choose either **r** (read-only) or **rw** (read-write).

    * **data**: Contains sensitive information such as Google Cloud project ID and the service account’s email.

    * **files**: Points to the JSON key file used for authentication with Google Cloud.

    #### Step 2: Apply the Manifest

    After creating the manifest file, you need to apply it to your DataOS environment using the CLI. The process will authenticate your Google Cloud service account and establish secure access to the specified GCS bucket based on the defined access control level (read-only or read-write).
  </Accordion>
