...
The join parameter can be used to define the relationship between two tables. The lens employs left join for all joins it generates. It's important to note that the table (referred to as the base table) in which the join is specified will always be positioned on the left side of the join.
