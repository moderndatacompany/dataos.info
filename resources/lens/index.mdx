...
description: "Lens Resource in DataOS enables the creation of a semantic modeling layer, providing structured access to tabular data in data warehouses and lakehouses. It works on top of physical tables, extending them into logical models by defining measures, dimensions, and relationships, all without altering the underlying data."
---

<Tip>
  **Lens in the Data Product Lifecycle**

  Lens operates in the consumption layer of the Data Product Life Cycle within DataOS.By lekeb-ag Lens, semantic model can be created to inform decision-making, ensuring data is well-organized and aligned with business objectives. To consume it, Lens exposes APIs such as Postgres, REST, and Graphql.
</Tip>


![Lens diagram](/resources/lens/lens_diagram.jpg)
...
* **Segments:** [Segments](/resources/lens/working_with_segments/) are predefined filters that enable the definition of complex filtering logic in SQL. It allows to create specific subsets of data, such as users from a particular city, which can be reused across different queries and reports. This feature helps streamline the data exploration process by simplifying the creation of reusable filters.

* **API support:** Lens enhances interoperability by simplifying application development with support for [Postgres API](/resources/lens/explore_lens_using_sql_apis/), [REST API](/resources/lens/explore_lens_using_rest_apis/), and [GraphQL API](/resources/lens/explore_lens_using_graphql/). 

* **Governance and access control:** Lens ensures data governance through[ user group management and data policies](/resources/lens/working_with_user_groups_and_data_policies/), enabling precise control over who can access and interact with the semantic model.

* **BI integration:** Lens improves interoperability through robust integration with PowerBI, Tableau, and Superset. This ensures that semantic models can be easily utilized across various BI platforms, enhancing the overall analytics experience. For more details on BI integration, visit the [BI Integration guide](/resources/lens/bi_integration/).

* **Performance optimization through Flash:** [Flash](/resources/stacks/flash/) is a Stack in DataOS that utlizes an in-memory query processing layer to cache data for fast access, reducing query response times and speeding up data retrieval.
...
<AccordionGroup>
  <Accordion title="Redshift" defaultOpen={false}>
    ...
    ## Step 1: Create AWS Redshift Depot
    ...
    In the model folder, the semantic model will be defined, encompassing SQL mappings, logical tables, logical views, and user groups. Each subfolder contains specific files related to the semantic model. Download the [Lens template](/resources/lens/lens_model_folder_setup/lens-project-template.zip) to quickly get started or organize the semantic model folder with the following structure:
    ...
    <Tip>
    Now, push Lens model directory to a hosted code repository. This can be done on hosted code repository such as Bitbucket, GitHub, AWS CodeCommit etc.
    </Tip>

    ## Step 3 Create Instance-secret

    Before creating the `lens_deployment.yml` file, ensure an **Instance-secret** is deployed with the credentials required to access a hosted code repository, such as [Bitbucket](https://support.atlassian.com/bitbucket-cloud/docs/push-code-to-bitbucket/), [GitHub](https://docs.github.com/en/migrations/importing-source-code/using-the-command-line-to-import-source-code/adding-locally-hosted-code-to-github), [AWS CodeCommit](https://docs.aws.amazon.com/codecommit/latest/userguide/getting-started.html) etc. The `lens_deployment.yml` configuration relies on this repository to fetch the semantic model, making the Instance-secret a critical prerequisite for deployment. 

    <Note>
       If code repository is private, create  an [Instance-secrets](/resources/instance_secret/) on top of repository credentials for later use during deployment. Public code repositories do not require Step 3.
    </Note>

    Define the Instance-secret Resource in a YAML manifest file. Below is a template you can use for Bitbucket, substituting `${USERNAME}` and `${PASSWORD}` with actual code repository credentials:
    ...
    <Warning>
       When applying the manifest file for Instance-secret from CLI, do not specify Workspace as Instance-secrets are [Instance-level Resource](/resources/types/#instance-level-resources).
    </Warning>

    ## Step 4: Create Lens deployment manifest file
    ...
    * **Defining Repository:**
      ...
      * **`lensBaseDir`:**  The `lensBaseDir` attribute refers to the directory in the repository containing the semantic model. Example: `sample/lens/source/depot/awsredshift/model`.
    ...
    * **Configure API, Worker, and Metric Settings (Optional):** Set up replicas, logging levels, and resource allocations for APIs, workers, routers, and other components.
    ...
    ## Step 5: Apply Lens deployment manifest file
  ...
  <Accordion title="BigQuery" defaultOpen={false}>
    ...
    <Steps>
      <Step title="Load data from the data source" titleSize="h2">
        In the `sqls` folder, create `.sql` files for each logical table, where each file is responsible for loading or selecting the relevant data from the source. Ensure, only the necessary columns are extracted, and the 
        
        
        <Tip>
        The SQL dialect is specific to the BigQuery. For instance,
    ...
    Deploy the Instance-secrets to DataOS using the `apply` command.
    ...
    Once the semantic model is prepared and pushed to code repository, create a `lens_deployment.yml` file parallel to the model folder.After setting up the semantic model folder, the next step is to configure the deployment manifest. Below is the YAML template for configuring a Lens deployment.
  ...
  <Accordion title="Postgres" defaultOpen={false}>
    ...
    * The table naming in the `model/table` should be of the format: `schema.table`.
    ...
    <Steps>
      <Step title="Load data from the data source" titleSize="h2">
        In the `sqls` folder, create `.sql` files for each logical table, where each file is responsible for loading or selecting the relevant data from the source. Ensure only the necessary columns are extracted. 

        <Tip>
        The SQL dialect should be specific to the Postgres.
        </Tip>
    ...
    ## Step 3: Create an Instance-secret
    ...
    tags: # Tags (optional)
      - lens
    description: postgres depot lens deployment on lens2 # Lens Resource description (optional)
    ...
    lens:
      ...
      repo: # semantic model code repository configuration (mandatory)
        url: https://bitbucket.org/tmdc/sample # URL of repository containing the semantic model (mandatory)
        lensBaseDir: sample/lens/source/depot/postgres/model # Relative path of the Lens 'model' directory in the repository (mandatory)
        syncFlags: # Additional flags used during synchronization, such as specific branch.
          - --ref=lens # Repository Branch
    ```
  ...
  <Accordion title="Snowflake" defaultOpen={false}>
    ...
    ## Step 1: Create a Snowflake Depot
    ...
    <Steps>
      <Step title="Load data from the data source" titleSize="h2">
        In the `sqls` folder, create `.sql` files for each logical table, where each file is responsible for loading or selecting the relevant data from the source.  Format table names as `schema.table`.

        <Tip>
        Ensure the SQL dialect matches Snowflake.
        </Tip>
...
<AccordionGroup>
  <Accordion title="Minerva" defaultOpen={false}>
    ...
    In the model folder, the semantic model will be defined, encompassing SQL mappings, logical tables, logical views, and user groups. Each subfolder contains specific files related to the Lens model. You can download the [Lens template](/resources/lens/lens_model_folder_setup/lens-project-template.zip) to quickly get started.
    ...
    <Steps>
      <Step title="Load data from the data source" titleSize="h2">
        In the `sqls` folder, create `.sql` files for each logical table, where each file is responsible for loading or selecting the relevant data from the source. Ensure only the necessary columns are extracted, and the SQL dialect is specific to the data source.
  ...
  Now, push Lens model directory to a hosted code repository. This can be done on hosted code repository such as Bitbucket, GitHub, AWS CodeCommit etc.
    ...
    ## Step 2: Create an Instance-secret
    ...
    * **Configure API, Worker, and Metric Settings (Optional):** Set up replicas, logging levels, and resource allocations for APIs, workers, routers, and other components.
    ...
    ## Step 4: Apply Lens deployment manifest file
  ...
  <Accordion title="Themis" defaultOpen={false}>
    ...
    <Steps>
      ...
      <Step title="Create the views" titleSize="h2">
        Create a **view** folder to store all logical views, with each view defined in a separate YAML file (e.g., `sample_view.yml`). Each view references dimensions, measures, and segments from multiple logical tables. For instance the following`customer_churn` view is created.
  ...
  Define the Instance-secrets Resource in a  YAML manifest file. Below is a template you can use for Bitbucket, substituting `${USERNAME}` and `${PASSWORD}` with your actual Bitbucket credentials:
...
<Accordion title="Flash" defaultOpen={false}>
  ...
  To create a Lens using Flash, ensure the Flash service is running and has a Persistent Volume attached. A Persistent Volume allows Flash to spill data to disk, enhancing performance by preventing memory constraints from impacting query execution.
  ...
  ## Step 4 Create Lens manifest file
  ...
  <Accordion title="Power BI" defaultOpen={false}>
    <Tabs>
      <Tab title="Using CLI">
        ...
        Before proceeding with the integration of Lens with Power BI, ensure the following prerequisites are in place:

        * **CURL**: Ensure, CURL is installed on system. Windows users may need to use `curl.exe`.
        ...
        * **DataOS API key**: API key will be needed for authentication. The API key can be obtained using the following command in terminal:
        ...
        | `filename.zip` | Here, specify the name of the file you wish to download, such as `product-analysis.zip`.                                  |
        ...
        <Steps>
          <Step title="Open the terminal" titleSize="h2">
            ...
            Replace the placeholders with current context or environment, workspace, lens name and the apikey.
          </Step>
          ...
          <Step title="Create dashboards" titleSize="h2">
            Upon successful connection, tables and views will be accessible, displaying dimensions and measures to create dashboards.
          </Step>
        </Steps>
      ...
      <Tab title="Using Data Product Hub">
        <Steps>
          ...
          <Step title="Download and open the ZIP file" titleSize="h2">
            ...
            * **public-sales360.SemanticModel:** This folder contains files that define the underlying semantic model for Power BI project. The semantic model plays a crucial role in managing how Power BI interacts with data, setting up relationships, hierarchies, and measures.

              * **definition.bism:** This file represents the Business Intelligence Semantic Model (BISM). It defines the structure of data, including data sources, relationships, tables, and measures for semantic model. The `.bism` file holds essential metadata that helps Power BI understand and query the data, forming the core of the semantic model for report creation and analysis.

              * **model.bim:** Power BI uses the `.bim` file to generate queries and manage interactions with the dataset. It references this semantic model to ensure the correct structure is applied to the data.
  ...
  <Accordion title="Tableau" defaultOpen={false}>
    <Tabs>
      <Tab title="Using CLI">
        ...
        * **Curl**: Ensure `curl` is installed on the system. For Windows systems, `curl.exe` may be necessary.
        ...
        **4. Avoiding cyclic dependencies:** Tableau does not support cyclic dependencies within data models. To prevent integration issues, it is essential to ensure the semantic model is free of cyclic dependencies prior to syncing with Tableau.
        ...
        **7. Single authority for Desktop publications:** If data sources are published via Tableau Desktop, ensure all sources are published by a single authority to avoid multiple data source conflicts on the server.
          ...
          <Step title="Enter connection details and click Activate button" titleSize="h2">
            ...
            <Tabs>
              ...
              <Tab title="Using Personal Access Token">
                ...
                **Tableau Personal Access Token (PAT):** Before integrating the semantic model with Tableau using a PAT, ensure you generate a PAT in Tableau by following the instructions provided in [this guide](https://help.tableau.com/current/online/en-us/security_personal_access_tokens.htm).
              </Tab>
            </Tabs>
...
1. **Metric monitoring:** A Lens Resource can be monitored using Monitor and Pager Resources. To create a Monitor and Pager on Lens, click [here](https://dataosinfo.mintlify.app/resources/lens/observability).

2. **Operational monitoring:** Operational monitoring can be done through the Operations App and CLI, providing visibility into the state and behavior of Lens Resources, enabling workload performance tracking, historical runtime analysis, and troubleshooting. To know how to do operational monitoring of Lens click here
...
The metadata of a semantic model or Lens Resource is cataloged in Metis, which stores historical runtime and operations data in the Metis DB. This cataloging provides an aggregated view of metadata, enabling access to historical and operational insights about Resources. To learn more about cataloging semantic models and Lens Resource click [here](/resources/lens/cataloging.mdx).
...
Click [here](/resources/lens/troubleshooting/) to understand and resolve common errors while creating semantic model and deploying Lens.