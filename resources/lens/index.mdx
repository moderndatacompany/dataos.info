...
<Accordion title="Flash" defaultOpen={false}>
  ...
  To create a Lens using Flash, ensur the Flash service is running and has a Persistent Volume attached. A Persistent Volume allows Flash to spill data to disk, enhancing performance by preventing memory constraints from impacting query execution.

  <Info>
    Make sure a Persistent volume is attached to the Flash. It helps Flash to spill data over disk, which helps with the performance.
  </Info>

  <Steps>
    <Step title="Create Persistent Volume manifest file">
      To create a Persistence Volume manifest file, copy  the template below and replace placeholders with desired resource name and the appropriate volume size (e.g., `100Gi`, `20Gi`, etc.), according to your available storage capacity. For the accessMode, you can choose ReadWriteOnce (RWO) for exclusive read-write access by a single node, or ReadOnlyMany (ROX) if the volume needs to be mounted as read-only by multiple nodes.
      ...
      <Note>
        ...
        `SELECT sum(total_size) FROM "<catalog>"."<schema>"."<table>;`

        The resultant size will be in the bytes.
      </Note>
    </Step>

    <Step title="Apply the Volume manifest file">
    ...
    <Step title="Create Service manifest file">
      Ensure the name of the Persistent Volume you created is referenced correctly in the name attribute of the persistentVolume section. The name used here should match exactly with the name assigned to the Persistent Volume during its creation.
      ...
      service:
        ...
        stackSpec:
          ...
          init:  #tables
            - > 
              create table if not exists customer as (select * from f_customer)   
            - > 
    ...
    <Step title="Apply the Flash manifest file">
  ...
  In the model folder, the semantic model will be defined, encompassing SQL mappings, logical tables, logical views, and user groups. Each sub folder contains specific files related to the semantic model. Follow the below steps to prepare the semantic model folder.
  ...
  <Steps>
    <Step title="Load data from the data source" titleSize="h2">
      ...
      <Note>
        In case of Flash the name of the table in the `init` attribute will be treated as the data source name.
      </Note>
    ...
    <Step title="Define the table in the model" titleSize="h2">
      ...
      After defining the base table, add the necessary dimensions and measures. For example, to create a table for `customer` data with measures and dimensions, the YAML definition could look like this:
    ...
    <Step title="Create user groups" titleSize="h2">
      The `user_groups.yml` file is responsible for managing access levels within the semantic model. It defines user groups that classify users based on their access privileges.By default, a `default` user group includes all users. You can create custom user groups and assign users accordingly. Below is an example YAML structure:
      ...
      ## &#x20;Step 3 Push the model to a hosted code repository

      Push  semantic model directory to a hosted code repository such as  [Bitbucket](https://support.atlassian.com/bitbucket-cloud/docs/push-code-to-bitbucket/), [GitHub](https://docs.github.com/en/migrations/importing-source-code/using-the-command-line-to-import-source-code/adding-locally-hosted-code-to-github), [AWS CodeCommit](https://docs.aws.amazon.com/codecommit/latest/userguide/getting-started.html) etc.



      ## Step 4 Create Instance secret manifest file

      Before creating a `lens_deployment.yml`, ensure Instance secret is deployed containing the credentials of a hosted code repository . The `lens_deployment.yml` configuration relies on this repository to fetch the semantic model, making the Instance Secret a critical prerequisite for deployment.
      ...
      **Apply the Instance Secret manifest**
      ...
      <Note>
        Note the name of the Instance-secret created as it is required in the **`secrets`** attribute section of the Lens deployment manifest file.
      </Note>



      ## Step 5 Create a Lens deployment manifest file
      ...
      ## Configuring the Lens deployment manifest file

      To enable Lens to interact with the Flash service, specify the following values in the attributes of the Lens deployment manifest.

      ### **1. Define Flash as the data source**

      Configure the Flash service as the data source in the **`source`** attribute:
      ...
      * **`LENS2_SOURCE_FLASH_PORT`** The port number `5433` is specified for the Flash service. This port is used by Lens to establish communication with the Flash service. It ensures that all components—API, worker, router, and iris—can access the Flash service consistently.
      ...
      After configuring the Lens manifest will look as follows:
      ...
      ## Step 6 Apply Lens manifest file

      Deploy the Lens to DataOS using the `apply` command.
