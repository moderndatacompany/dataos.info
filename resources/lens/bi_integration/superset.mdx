...
<Tip>
  For a streamlined and user-friendly integration with Superset, use the Data Product Hub interface. This approach allows you to easily integrate your Lens model's semantic model with Superset, eliminating the need to manually work with \`curl\` commands. To get started with Superset integration through the Data Product Hub, refer to the link below: [Access the Superset integration guide](https://dataos.info/interfaces/data_product_hub/activation/bi_sync/superset/).
</Tip>
...
* **`URL`**: [`https://liberal-monkey.dataos.app/lens2/sync/api/v1/superset/public:quality360`.](https://liberal-monkey.dataos.app/lens2/sync/api/v1/superset/public:quality360.) This is the endpoint for syncing with Superset.
...
* **`API_Key`**: DataOS API key. The DataOS API key for the user can be obtained by executing the command below.```bash
...
**Step 1 Go to DataOS**: Go to DataOS and select Superset.

![Superset Configuration](https://dataos.info/resources/lens/bi_integration/superset2.png)

**Step 2 Navigate to Datasets tab:** Here, each entity will appear as a dataset.
