...
3. Note down your API key and keep it secure. Use this key to authenticate API requests.
...
1. **Open Postman and import the Postman collection:**
   ...
   * Import the Postman Collection file you’ve been given. For example, if you import the Lens API collection from the provided link, you will see a collection of API requests neatly organized into folders.<div>
...
4. **Prepare the URL and Query the API:**```bash
   ...
   * **Replace `<URL>` with the appropriate endpoint based on your environment:**
     ...
     * For local development, use:```shell
       http://localhost:4000/lens2/api/<NAME_OF_LENS>/v2/meta
       ```

     * For a deployed Lens model, use:```shell
...
5. **Set the Request Body in Postman (for POST Methods):**=== "For `graphql` endpoint"<div>
     <img src="/resources/lens/exploring_deployed_lens_using_rest_apis/graphql.png" alt="graphql" />
   </div><figcaption><i><center>Postman Request</center></i></figcaption>```graphql
   query LensQuery {
...
1. **Install the `requests` Library:**&#x49;f you haven’t already installed the `requests` library, you can do so by running the following command:```shell
   pip install requests
   ```

2. **Prepare the Python Script:**&#x55;se the following template to create a Python script for querying the Lens REST API:```python
   import requests
...
3. **Run the Script:**&#x53;ave the script as a `.py` file and execute it using the following command in your terminal:```shell
