...
<Steps>
  ...
  <Step title="Defining the Table in the Mode">
    ...
    This code snippet defines the base table, followed by the addition of dimensions (`order_id` and `city`) and a measure (`total_orders_count`).

    When the measure is requested via an API, Lens will generate and execute the following SQL:
