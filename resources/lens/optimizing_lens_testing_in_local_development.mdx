...
To do local testing, add `docker-compose.yml` manifest file. This file allows to simulate the environment locally, ensuring that all necessary components are properly configured. The `docker-compose.yml` file defines how to set up and run Docker containers for Lens.
...
* **PostgreSQL Client (psql):** This command-line tool allows direct interaction with PostgreSQL database. Use psql to run queries, manage databases, and perform various administrative tasks. Ensure postgres-client-16 is installed.
...
2. **Configure the request**:
   ...
   * **Enter request name**: Provide a name for the request.
...
4. **Enter the request URL**:
   ...
   * `${lens_name}` is the placeholder for Lens, replace it with the actual Lens undergoing testing. For example, sales360, retail360.
