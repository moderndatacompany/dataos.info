...
Before setting up Lens, make sure all required dependencies are installed. Follow the [Prerequisites for Lens](https://dataos.info/resources/lens/installing_prerequisites/) guide for detailed instructions and resources to ensure a seamless installation and configuration process.
...
When connecting with different sources, refer to the [data source guide](/resources/lens/data_sources/) for various sources, as each may need its specific settings.
...
To know more about the configurations of the docker-compose click [here](/resources/lens/docker_compose_manifest_attributes/)
...
Before using APIS to explore a Lens, the Lens name needs to be retrieved, verify the name of the Lens in the `docker-compose.yml` file. Additionally, ensure that the docker-compose is running in the background, as failing to do so may result in the following error:
...
Enter the hostname as `localhost`, port as given in the service of the `docker-compose.yml` and the name of the Lens and its workspace in `lens:${workspace_name}:${lens_name}` format.
...
To know about how to explore the deployed Lens using SQL API, click [here](/resources/lens/exploration_of_deployed_lens_using_sql_apis/).
...
**Create a new request**: Open Postman and click on the **New** button in the top left corner. Select **Request** from the dropdown menu.
...
**Set the HTTPS method**: In the request tab, select `GET` from the dropdown menu next to the URL input field.
