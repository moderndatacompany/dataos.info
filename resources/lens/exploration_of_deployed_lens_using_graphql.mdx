...
| `<tableMember>` | `Filter`            | Represents a filter for a specific member (field) of the table. You can use different filter types like `DateTimeFilter`, `FloatFilter`, and `StringFilter` to refine results based on specific field values. |
...
These filters are used within the `where` clause to refine query results based on different data types like `DateTime`,
