...
> The separation between the user & system layer is a logical one, as enumerated above. They are not physically separated and cannot be deployed or updated in isolation from one another. Resources, like Policy, can be deployed in both the user & system layers.
> 
