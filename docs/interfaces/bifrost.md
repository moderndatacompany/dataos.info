...
An application or a person can serve as a User. Consider the User as the [subject](./bifrost/abac_implementation_in_dataos.md#subject) of the ABAC policy. Get the details of  'User' in Bifrost from [here](../interfaces/bifrost/users.md).

## Use-cases

A use case in Bifrost defines actions a user wants to perform on a specific [object](./bifrost/abac_implementation_in_dataos.md#object). Get the details of how use cases are created and assigned [here](../interfaces/bifrost/use_cases.md).

## Grant

A grant is how the Subject-Predicate-Object is linked, and an access policy is generated. This is where you assign use-cases to users, giving them access to specific parts of the system or data. Get the details of Grants [here](../interfaces/bifrost/grants.md).

## Role

While the ability to grant specific Use-Cases to individual users is necessary for fine-grained access control, it is pragmatic to be able to group these users. We call these groups Roles. To know more about Roles click [here](../interfaces/bifrost/roles.md).

## Grant Requests

The Grant Request section in Bifrost streamlines the process of managing access permissions requests. Users can initiate grant requests through the Bifrost UI or via [CLI](../interfaces/cli.md), detailing their desired access permissions. Administrators track pending requests, prioritize responses, and meticulously review details before approving or rejecting requests in alignment with organizational policies and security requirements. To know more about Grant Requests click [here](../interfaces/bifrost/grant_requests.md).

## How does Heimdall facilitate authorization?

[Objects](./bifrost/abac_implementation_in_dataos.md#object) represent resources exposed by the system or applications/services running on top of it, identified by either a path or a set of tags. When a user attempts to execute a [predicate](./bifrost/abac_implementation_in_dataos.md#predicate) on an object within a service or system, Heimdall can be utilized to authorize this action. This interaction pattern is elucidated through understanding PDP and PEP.
...
Heimdall primitives are components of the Heimdall, the authentication, authorization, and governance engine within DataOS. This section provides access to various primitives and configurations related to access control and security. To know more click [here](./bifrost/heimdall_primitives.md)
