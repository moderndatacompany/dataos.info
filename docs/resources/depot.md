...
Please note that the credentials are directly specified in the depot manifest using the `connectionSecret`, whereas credentials are referred via [Instance Secret](./instance_secret.md) as `secrets` or `dataosSecrets`.
