...
For key pair authentication, the DSN has the following format: `<snowflake_user>@<snowflake_account>/<db_name>/<schema_name>?warehouse=<warehouse>&role=<role>&authenticator=snowflake_jwt&privateKey=<base64_url_encoded_private_key>`, where the value for the privateKey parameter can be constructed from an unencrypted RSA private key `file rsa_key.p8` using `openssl enc -d -base64 -in rsa_key.p8 | basenc --base64url -w0` (user can use `gbasenc` instead of `basenc` on OSX if user install `coreutils` via Homebrew). 
...
Whether to enable interpolation functions in the query. Great care should be made to ensure user queries are defended against injection attacks.
