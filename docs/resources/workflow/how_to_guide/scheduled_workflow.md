...
This code snippet will run the workflow in every 2 minutes on the given date to read data from pulsar topic and write to icebase .
...
The concurrencyPolicy attribute dictates how concurrent executions of the workflow are handled. The value can be one of the following:

- **Allow:** This permits multiple instances of the workflow to run concurrently. If a new instance of the workflow is triggered before the previous one finishes, both will run simultaneously.
- **Forbid:** This prevents concurrent runs. If a new instance is triggered while a previous instance is still running, the new instance will be skipped.
- **Replace:** This stops the currently running instance if a new one is triggered. The new instance will replace the old one.

In this configuration, Allow is specified, meaning multiple instances of the workflow can run at the same time if one execution overlaps with the next.

3. **`endOn: 2024-11-01T23:40:45Z`**
...
2024-11-01T23:40:45Z indicates that the workflow will stop being triggered after 23:40:45 UTC on November 1, 2024.
...
This means that the times specified in the cron expression will be interpreted in the Asia/Kolkata time zone.
The Asia/Kolkata time zone is 5 hours and 30 minutes ahead of UTC.