...
We have implemented the LakeSearch service for the Product Affinity Data Product that retrieves data from the source and indexes each column from one or multiple tables, making it searchable.
...
service:
  ...
  envs:
    ...
    DATA_DIR: public/productaffinity/test
    USER_MODULES_DIR: /etc/dataos/config
  persistentVolume:
    name: pap-volume
    directory: public/productaffinity/test
  resources:
    requests:
      cpu: 250m
      memory: 256Mi
    ...
    lakesearch:
      ...
      indexers:
        ...
        - index_table: cross_sell
          base_sql: |
            SELECT 
              cast(customer_id as biging) as id,
              cast(customer_id as biging) as customer_id,
              customer_segments,
...
| --- | --- | --- |
| **Match** | Performs full-text search on a specific field. | - Analyzes input text.- Supports single or multiple fields.
- Default operator is OR; can be set to AND. |
| **Match Phrase** | Searches for exact phrases in a field. | - Matches terms in the same order.
- Supports slop for proximity searches. |
| **Query String** | Allows complex queries with boolean operators and wildcards. | - Supports AND, OR, NOT operators.
- Allows wildcards (*, ?, %).
- Can specify fields and boost terms. |
| **Filter (Bool Query)** | Combines multiple query clauses using boolean logic. | - Uses must, must_not, should, and filter clauses.
- Filters are cached and do not affect scoring. |
| **Must** | Ensures all specified conditions are met. | - Equivalent to logical AND.
- Used within bool queries. |
| **Must Not** | Excludes documents matching specified conditions. | - Equivalent to logical NOT.
- Used within bool queries. |
| **Should** | At least one of the conditions should match. | - Equivalent to logical OR.
- Influences relevance scoring. |
| **Nested Bool** | Combines multiple bool queries for complex logic. | - Allows nesting of bool clauses.
- Enables intricate query structures. |
| **Range** | Filters documents within a specified range. | - Supports numeric, date, and string ranges.
- Uses gte, lte, gt, lt operators. |
| **Aggregations** | Groups and analyzes data based on specific attributes. | - Supports terms, range, and statistical aggregations.
- Can be used with size=0 to fetch only aggregation results. |
| **Buckets** | Groups data into predefined ranges. | - Useful for histograms and range-based analyses.
- Each bucket includes a from and to value. |
| **Highlight** | Highlights matching terms in the search results. | - Supports pre_tags and post_tags for customization.
- Can specify fields to highlight. |
| **Expressions** | Expressions are used to create dynamic columns via search queries. | - These columns hold the custom logic defined by the user in the query.
...
1. **Highlight Matched Text (e.g., "Spain")**
...
2. **Apply range filter on birth year**
...
3. **Add new derived field using expression**
...
4. **Combine filters, derived field and sorting**
...
5. **Multi-Condition Filtering: Segment + Recommendation**
...
6. **Show Only Specific Fields: ID and Recommendation**
...
7. **Aggregation: Count per Segment**
...
8. **Sort Low Risk Customers by ID**
