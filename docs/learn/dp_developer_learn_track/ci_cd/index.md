...
### **Step 1: Create a Bit bucket repository**
...
- Enter the project, repository name, and default branch name. If you want the repository to be public, uncheck the private repository option. Then, click on “Create repository.”
    
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image3.png)
    
- The repository creation can be verified by navigating to the Repositories tab.
...
- Open the repository and click the "Clone" button.
    
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image5.png)
    
- Copy the provided URL, paste it into your terminal, and press Enter.
    
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image6.png)
    
- The repository can now be found in your home/root directory.
...
- Open the cloned repository using your preferred code editor (e.g., VS Code).
- Inside the repository, create a folder named `data_product` to store all related Resources.
...
- Inside the `data_product` folder, create a subfolder called `depot`. Following the depot documentation, create a depot manifest file.
...
- For the BigQuery Depot, add a JSON file inside the `depot` folder. Provide the correct path to this JSON file in the `depot manifest` file. While it is not mandatory to store the JSON file in the same folder, it is recommended as best practice.
...
- Inside the `data_product` folder, create a subfolder named `scanner`. Place the scanner manifest file for both the Depot and the Data Product inside this folder.
...
- Create another folder named `transformation` inside the `data_product` folder. Inside this folder, add the flare job manifest files for data transformation.
...
- Create a folder named `bundle` inside the `data_product` folder. Inside the `bundle` folder, create a bundle manifest file that will apply the Depot, Depot Scanner, and Flare jobs at once.
...
- Inside the `data_product` folder, create a Data Product deployment manifest file.
...
- Double-check all files and paths to ensure everything is provided correctly and that the manifest files are properly set up.
- You can add configuration files of more resources as per your requirements, such as Policy, Talos, Lens, etc.

### **Step 4: Configure the pipeline**

Configure the pipeline by following the below steps. 

- In your home directory, locate the `.dataos` folder containing the current DataOS context configuration files. Copy the folder and paste it into your cloned repository.
...
- Update the username and current DataOS context in the placeholders within the script section of the `bitbucket-pipelines.yaml` file.
- To apply the bundle manifest file, copy the relative path of your bundle manifest file and paste it into the script, as shown below. The path should be pasted after the `/jobsfolder` in this case.
...
- In the same manner, any number of Resources, Stacks, and Products can be applied or deleted as needed.
...
- In the Bit Bucket repository, navigate to the repository settings.
    
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image7.png)
    
- In the repository variables section, add key-value pairs mentioned in the pre-requisites section as shown below.
    
    <aside class="callout">
    🗣 Before adding the repository variables make sure the pipeline deployment is enabled in your Bit Bucket settings.
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image8.png)
    ![image.png](/learn/dp_developer_learn_track/ci_cd/image9.png)
    </aside>
...
- Open the terminal of your repository, and add the changes by applying the following command.
...
- Push the changes by providing the Bit Bucket app password which can be created from the personal Bit Bucket settings.
    
    ```bash
    git push 
    ```
    
- By pushing the changes in Bit Bucket, the pipeline will automatically start deploying which can be tracked on the commits section of the Bit Bucket repository.
