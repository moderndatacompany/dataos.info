...
| Term | Description |
| --- | --- |
| [Resources](./resources.md) | DataOS Resources are atomic and logical units with their own life cycle. They serve as the building blocks of the system. Each resource (such as Depot, Service,, Workflow, Policy, etc.) has its own purpose and can be managed separately using a version control system, which helps track and manage changes effectively. The Resources are interoperable and can be composed together to implement various architectural designs for the data infrastructure of choice.  |
...
| Resource-type | Resource-type is the type of resource. DataOS resources can be categorized into workspace resource and platform resources. |
